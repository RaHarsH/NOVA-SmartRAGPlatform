      ...
      <div className="flex items-center justify-between p-4 border-b border-white/10 bg-black">
        ...
        <div className="flex items-center space-x-2">
          ...
          <Button
            ...
            variant="ghost"
            size="sm"
            className="curdor-pointer text-gray-400 hover:text-white"
          >
      ...
      <div className="flex-1 bg-black relative overflow-auto">
        {isLoading && (
          <div className="absolute inset-0 flex flex-col items-center justify-center space-y-4 animate-pulse z-10">
            <Skeleton className="flex justify-center items-center w-full h-32 rounded-lg bg-gray-800/20">
        ...
        {!isLoading && !error && csvData.length > 0 && showPreview && (
          <div className="p-4">
            <div className="bg-black/50 rounded-md border border-gray-800 overflow-hidden">
                  ...
                  <thead>
                    <tr className="bg-gray-800/30 border-b border-gray-800/50">
                      {csvData[0]?.map((header, index) => (
                        <th
                          key={index}
                          className="px-3 py-2 text-left font-medium text-gray-300 border-r border-gray-800/50 last:border-r-0 min-w-[100px]"
                        >
                    ...
                    {displayRows.slice(1).map((row, rowIndex) => (
                      ...
                      >
                        {row.map((cell, cellIndex) => (
                          <td
                            key={cellIndex}
                            className="px-3 py-2 text-gray-300 border-r border-gray-700/40 last:border-r-0 max-w-[200px] truncate"
                            title={cell}
                          >
                            {cell || '-'}
                          </td>
